...
import { isBehindPlane, type Plane } from "./plane.js";
...
const isPointNotInsideConvexHull = (point: vec3.Vec3, points: vec3.Vec3[]): boolean => {
    const center = vec3Avg(points);
    if (vec3.almostEquals(point, center)) return true;
    const dir = vec3.sub(point, center);
    const plane: Plane = {
        pos: point,
        dir
    };
    return points.every(p => p === point || isBehindPlane(p, plane));
};
